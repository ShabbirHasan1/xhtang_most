...
int main()
{
    ...
    for (int i_factor = 0; i_factor < n_factor; ++i_factor)
    {
        for (int i = 1; i <= N; i++)
        {
            ...
            {
                num[i_factor][i] = (num[i_factor][i] * 10 + factor_t(buffer[j] - '0')) % factors[i_factor];
            }
        }
    }
        ...
        while (pos + 1024 < BUFFER_SIZE)
        {
            ...
            for (int i = pos; i < pos + n; i++)
            {
                ...
                for (int i_factor = 0; i_factor < n_factor; ++i_factor)
                {
                    for (int k = 1; k <= N; k++)
                    {
                        num[i_factor][k] = (num[i_factor][k] * 10 + ch + factors[i_factor] - factor_t(buffer[i - k] - '0') * pow10[i_factor][k] % factors[i_factor]) % factors[i_factor];
                    }
                }
                int ans_k = -1;
                for (int k = 1; k <= N; k++)
